...
def search(method='GET'):
    ...
    # demultiplex filters
    filters = {field: value.split('|') for field, value in request.args.items(
    ) if field not in ["query", "size", "offset", "date"]}
    ...
    for f, values in filters.items():
        filter_must = {"bool": {"should": [
            {"term": {f: v}} for v in values], "minimum_should_match": 1, "boost": 2}}
...
def autocomplete(method='GET'):
    ...
    if query:
        ...
        terms["include"] = ".*%s.*" % ("".join(["[%s%s]" %
                                                (c.lower(), c.upper()) for c in query]))
    es = Elasticsearch('%s:%s' % (ELASTICSEARCH_HOST, ELASTICSEARCH_PORT))
    search = {
        "size": 0,
        "track_total_hits": True,
...
def user_tags_csv(method='GET'):
    ...
    writer = csv.DictWriter(
        csvString, ["document_id", "text_segment_id", "user_tags"])
    writer.writeheader()
